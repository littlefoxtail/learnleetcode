...
fun maxProfit(price: IntArray): Int {
  ...
  val another = price.copyOfRange(minIndex, price.size)

  var anothermax = another[0]
  for (i in another.indices) {
    if (another[i] > anothermax) {
      anothermax = another[i]
    }
  }

  return anothermax - min
...
fun main(args: Array<String>) {
  ...
  val deciPrice = intArrayOf(7,6,5,4, 3,1)
  val max = maxProfit(deciPrice)
  print(max)
